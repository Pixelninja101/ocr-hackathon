...
def is_line_duplicate(line: str, existing_lines: list[str], threshold: float = 0.70) -> bool:
...
# Primary variant with highest score
ocr_results.sort(key=lambda r: r["score"], reverse=True)
...
for secondary in ocr_results[1:]:
    sec_lines = [l for l in secondary["text"].splitlines() if l.strip()]
    # Group words in secondary by line_id (block, par, line)
    sec_line_word_map = {}
    ...
    for lid, line_words in sec_line_word_map.items():
        ...
        if not is_line_duplicate(line_str, merged_lines):
            supplemental_lines.append(line_str)
...
print("\n--- Field Extraction on Merged OCR Result ---")
